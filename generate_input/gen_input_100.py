...
for i in range(1, 100+1):
    ...
    with open(inp_path, 'w') as f:
        inp_n = np.random.choice(list(range(ord('a'), ord('z')+1))+list(range(ord('A'), ord('Z')+1))+list(range(ord('0'), ord('9')+1)), size=np.random.randint(1, 50, size=1)[0])
