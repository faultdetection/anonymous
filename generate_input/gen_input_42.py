...
prog_id = '42' # input an integer vector and a value, output the vector except the value
...
for i in range(1, 100+1):
    ...
    with open(inp_path, 'w') as f:
        ...
        for _ in range(inp_n):
            inp_k = np.random.randint(-5, 5, size=1)[0]
            f.write('{}\n'.format(inp_k))
        inp_v = np.random.randint(-5, 5, size=1)[0]
        f.write('{}\n'.format(inp_v))
