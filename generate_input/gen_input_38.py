...
prog_id = '38' # input a few vectors, calculate deviation of each vector
...
for i in range(1, 100+1):
    ...
    with open(inp_path, 'w') as f:
        ...
        for _ in range(inp_n):
            inp_k = np.random.randint(0, 5, size=1)[0]
            f.write('{}\n'.format(inp_k))
            for k_i in range(inp_k):
                inp_v = np.random.randint(-20, 20, size=1)[0] / 4.0
                f.write('{}\n'.format(inp_v))
